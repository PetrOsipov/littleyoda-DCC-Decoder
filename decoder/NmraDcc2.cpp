// This Sourcecode is based on https://github.com/mrrwa/NmraDcc.
// It was modified to receive DccSpeed, DccFunc and DccTurnOut with one instance.
// CV Programming and using of the eeprom/flash is disabled!
//
//
//
//
...
// Bit evaluation without Timer 0 ------------------------------
	...
	if (bitMicros < bitMin) {
		...
		CLR_TP3;SET_TP4;SET_TP4;CLR_TP4;
		return; //>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> abort IRQ
	}
	...
	case WAIT_PREAMBLE:
		if (DccBitVal) {
			...
			if (DccRx.BitCount > 10) {
				...
				attachInterrupt(DccProcState.ExtIntNum,
						ExternalInterruptHandler, CHANGE);
				halfBit = 0;
	...
	case WAIT_START_BIT:
		...
		case 1: //SET_TP1; // previous halfbit was '1'
			if (DccBitVal) {
				...
				if ( abs(bit2-bit1) > MAX_BITDIFF) {
					...
					DccRx.BitCount = 0;
					attachInterrupt(DccProcState.ExtIntNum,
							ExternalInterruptHandler, ISREdge);
					SET_TP3;
				}
			} else {
		...
		case 3: //SET_TP1;  // previous halfbit was '0'  in second halfbit  
			...
			attachInterrupt(DccProcState.ExtIntNum, ExternalInterruptHandler,
					ISREdge);
			CLR_TP1;
			break;
		case 4:
			SET_TP1; // previous (first) halfbit was 0
...
uint16_t getMyAddr(void) {
	...
	if (CV29Value & CV29_ACCESSORY_DECODER)  // Accessory Decoder?
		Addr = (readCV( CV_ACCESSORY_DECODER_ADDRESS_MSB) << 6)
				| readCV( CV_ACCESSORY_DECODER_ADDRESS_LSB);

	else   // Multi-Function Decoder?
	{
		if (CV29Value & CV29_EXT_ADDRESSING)  // Two Byte Address?
			Addr = ((readCV( CV_MULTIFUNCTION_EXTENDED_ADDRESS_MSB) - 192) << 8)
					| readCV( CV_MULTIFUNCTION_EXTENDED_ADDRESS_LSB);
...
// HACK TO ALLOW DccSpeed, DCCFunc, and DccTurnout at the same time
...
//		if ((CmdMasked != 0b11100000)
//				|| (DccProcState.OpsModeAddressBaseCV == 0))
//			return;
//
//		uint16_t FakeOpsAddr = readCV(DccProcState.OpsModeAddressBaseCV)
//				| (readCV(DccProcState.OpsModeAddressBaseCV + 1) << 8);
	...
	case 0b01100000:
		...
		if (notifyDccSpeed) {
			...
			default:
				...
				{
#endif
				speed = (((Cmd & 0b00001111) << 1) | ((Cmd & 0b00010000) >> 4))
						- 2; // => 2..29
#ifdef NMRA_DCC_ENABLE_14_SPEED_STEP_MODE
					}
#endif        
	...
	if (pDccMsg->Size == 3) // 3 Byte Packets are for Address Only, Register and Paged Mode
		...
		if (RegisterAddr == 5) {
			...
			if (RegisterAddr == 4)
				...
				CVAddr = ((DccProcState.PageRegister - 1) * 4) + RegisterAddr
						+ 1;
...
#ifdef NMRA_DCC_PROCESS_SERVICEMODE
		if (DccProcState.inServiceMode && (pDccMsg->Data[0] >= 112)
				&& (pDccMsg->Data[0] < 128)) {
			resetServiceModeTimer(1);
			...
			// Multi Function Decoders (7-bit address)
				...
				processMultiFunctionMessage(pDccMsg->Data[0], DCC_ADDR_SHORT,
						pDccMsg->Data[1], pDccMsg->Data[2], pDccMsg->Data[3]);

			// Basic Accessory Decoders (9-bit) & Extended Accessory Decoders (11-bit)
			else if (pDccMsg->Data[0]
					< 192)
#else
				...
				if (DccProcState.Flags & FLAGS_DCC_ACCESSORY_DECODER) {
					...
					BoardAddress = (((~pDccMsg->Data[1]) & 0b01110000) << 2)
							| (pDccMsg->Data[0] & 0b00111111);
					...
					if (pDccMsg->Data[1] & 0b10000000) {
						...
						uint8_t outputPower = (pDccMsg->Data[1] & 0b00001000)
								>> 3;

						if (notifyDccAccState)
							notifyDccAccState(Address, BoardAddress,
									OutputAddress,
									pDccMsg->Data[1] & 0b00001000);

						if (notifyDccAccTurnoutBoard)
							notifyDccAccTurnoutBoard(BoardAddress, OutputIndex,
									direction, outputPower);

						if (notifyDccAccTurnoutOutput)
							notifyDccAccTurnoutOutput(Address, direction,
									outputPower);
					}

					else {
						if (notifyDccSigState)
							notifyDccSigState(Address, OutputIndex,
									pDccMsg->Data[2]);
					}
				}
			}
			...
			// Multi Function Decoders (14-bit address)
				...
				processMultiFunctionMessage(Address, DCC_ADDR_LONG,
						pDccMsg->Data[2], pDccMsg->Data[3], pDccMsg->Data[4]);
			}
#endif
#ifdef NMRA_DCC_PROCESS_SERVICEMODE
		}
...
void NmraDcc::pin(uint8_t ExtIntNum, uint8_t ExtIntPinNum,
...
void NmraDcc::initAccessoryDecoder(uint8_t ManufacturerId, uint8_t VersionId,
		uint8_t Flags, uint8_t OpsModeAddressBaseCV) {
	init(ManufacturerId, VersionId, Flags | FLAGS_DCC_ACCESSORY_DECODER,
...
void NmraDcc::init(uint8_t ManufacturerId, uint8_t VersionId, uint8_t Flags,
	...
	uint8_t cv29Mask = Flags
			& (CV29_ACCESSORY_DECODER | CV29_OUTPUT_ADDRESS_MODE); // peal off the top two bits
